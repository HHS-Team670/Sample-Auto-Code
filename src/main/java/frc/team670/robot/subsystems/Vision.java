...
public class Vision implements Subsystem, HealthySubsytem, DebugSubsytem {
    ...
    private PhotonTrackedTarget getAprilTag(
        ...
        try {
            Logger.recordOutput("Vision/STATUS", "GOOD");
            ...
            if (goodCamera == null) {
                Logger.recordOutput("Vision/STATUS", "GoodCameraNull");
                return null;
            }
            ...
            if (!results.isEmpty()) {
                        ...
                        for (var target : targets) {
                            for (int aprilTagNumber : allowedAprilTags) {
                                if (target.getFiducialId() == aprilTagNumber) {
                                    ...
                                    Logger.recordOutput("Vision/AprilTag" + aprilTagNumber + "Angle", angle);
                                }
                            }
                        }
                    } else {
                ...
                Logger.recordOutput("Vision/STATUS", "RESULTS EMPTY");
            }
            return bestTarget;
        } catch (java.lang.IllegalArgumentException e) {
            Logger.recordOutput("Vision/STATUS", "CATCH");
        }
        return null;
    }
    ...
    public void debugSubsystem() {
        Logger.recordOutput("Vision/AligningToAprilTag", AlignToClosestAprilTag.AligningToAprilTag);
        if (allowedAprilTags == null) {
            Logger.recordOutput("Vision/AllowedAprilTags", "NULL");
        } else {
            Logger.recordOutput("Vision/AllowedAprilTags", allowedAprilTags);
        }
    }
        ...
        for (PhotonCamera camera : mCameras) {
            if (camera == null || !camera.isConnected()) {
                ...
                if (camera.getName() == "ArducamL") {
                    Logger.recordOutput("ArducamL_Connected", true);
                }
            } else {
                if (camera.getName() == "ArducamR") {
                    Logger.recordOutput("ArducamR_Connected", true);
                }
            }
        }
